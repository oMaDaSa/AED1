...
Lista* interseccao(Lista *l1, Lista *l2){
    Lista *l3 = criar();
    No *no1 = l1->inicio, *no2;
    while(no1->prox != l1->inicio){
        no2 = l2->inicio;
        while(no2->prox != l2->inicio){
            if(no1->valor.mat == no2->valor.mat){
                inserirFim(l3, no1->valor);
                break;
            }
            no2 = no2->prox;
        }
...
int inserirFimSemRepeticao(Lista *l, Aluno it){
    ...
    if (listaVazia(l) == 0)
        inserirFim(l, it);
    else if(contaOcorrencia(l, it) == 0)
        inserirFim(l, it);
    return 0;
}